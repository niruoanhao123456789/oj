...
-- Seed the first admin (hash/salt generated by the application at runtime):
-- INSERT INTO users (username, password_hash, salt, role) VALUES ('admin', '<hash>', '<timestamp>', 'admin');
...
## 12. Roles, Groups & Question Management (planned)

> This entire feature set is **planned** (see [SPEC.md §14](SPEC.md#14-待办清单todo)) — not yet wired into the gateway. The database tables are defined in [Database Setup](#3-database-setup); the work folders `oj_server/user/` and `oj_server/question_manage/` are placeholders for the future implementation. All frontend pages use plain HTML + CSS + JS.
